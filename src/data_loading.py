...
def load_sms_spam_ds(data_root):
    texts, labels = [], []
    with open(os.path.join(data_root, 'datasets/sms_spam', 'sms_spam_dataset'), encoding = "utf-8") as f:
        for line in f:
...
def load_tweets_ds(data_root):
    texts, labels = [], []
    with open(os.path.join(data_root, 'datasets/disaster_tweets', 'train.csv'), encoding = "utf-8") as csv_file:
        for row in csv.DictReader(csv_file, delimiter=','):
            texts.append(row['text']), labels.append(int(row['target']))
    with open(os.path.join(data_root, 'datasets/disaster_tweets', 'test.csv'), encoding = "utf-8") as csv_file:
        test_data = {row['id']: row['text'] for row in csv.DictReader(csv_file, delimiter=',')}
    with open(os.path.join(data_root, 'datasets/disaster_tweets', 'sample_submission.csv'), encoding = "utf-8") as csv_file:
...
def load_emotions_ds(data_root):
    texts, labels = [], []
    with open(os.path.join(data_root, 'datasets/emotions', 'train.txt'), encoding = "utf-8") as f:
        for line in f:
            texts.append(line.split(';')[0].strip()), labels.append(line.split(';')[1].strip())
    with open(os.path.join(data_root, 'datasets/emotions', 'val.txt'), encoding = "utf-8") as f:
        for line in f:
            texts.append(line.split(';')[0].strip()), labels.append(line.split(';')[1].strip())
    with open(os.path.join(data_root, 'datasets/emotions', 'test.txt'), encoding = "utf-8") as f:
        for line in f:
            texts.append(line.split(';')[0].strip()), labels.append(line.split(';')[1].strip())
    return texts, labels
...
def load_imdb_ds(data_root):
    texts, labels = [], []
    with open(os.path.join(data_root, 'datasets/imdb', 'imdb_dataset.csv'), encoding = "utf-8") as csv_file:
        for row in csv.DictReader(csv_file, delimiter=','):
...
def load_bbc_ds(data_root):
    texts, labels = [], []
    with open(os.path.join(data_root, 'datasets/bbc', 'bbc_articles.csv'), encoding = "utf-8") as csv_file:
        for row in csv.DictReader(csv_file, delimiter=','):
...
def load_glove_embedding_vec(data_root, dim):
    ...
    with open(os.path.join(data_root, f"glove/glove.6B.{dim}d.txt"), encoding = "utf-8") as f:
